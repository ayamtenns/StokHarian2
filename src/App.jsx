          ...
          <>
            ...
            <div style={card}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <button onClick={() => setForm(f => ({ ...f, jumlah: String(Math.max(0, (Number(f.jumlah) || 0) - 0.5) || "") })) } style={qBtn}>−</button>
                <input type="number" inputMode="decimal" step="0.5" value={form.jumlah} onChange={e => setForm(f => ({ ...f, jumlah: e.target.value }))}
                  placeholder="Jumlah" style={{ ...inp, flex: 1, textAlign: "center", fontSize: 22, fontWeight: 900 }} />
                <button onClick={() => setForm(f => ({ ...f, jumlah: String((Number(f.jumlah) || 0) + 0.5) }))} style={{ ...qBtn, background: "#CC2200", color: "#fff", border: "none" }}>＋</button>
              </div>
