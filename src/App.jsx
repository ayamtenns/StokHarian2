...
export default function App() {
  ...
  const tambahKeKeranjang = () => {
    ...
    if (!form.itemId || !form.jumlah || isNaN(jumlahVal) || jumlahVal <= 0) return;
    const item = barang.find(b => b.id === form.itemId);
    if (!item) return;
    const existing = keranjang.findIndex(k => k.itemId === form.itemId);
    if (existing >= 0) {
      setKeranjang(k => k.map((x, i) => i === existing ? { ...x, jumlah: x.jumlah + jumlahVal, catatan: form.catatan || x.catatan } : x));
    } else {
      setKeranjang(k => [...k, { id: Date.now().toString(), itemId: form.itemId, nama: item.nama, satuan: item.satuanBeli, jumlah: jumlahVal, catatan: form.catatan }]);
    }
  ...
  // Helper: retry request sampai 3x
  const postWithRetry = async (body, maxRetry = 3) => {
    for (let i = 0; i < maxRetry; i++) {
      try {
        const d = await gasPost(scriptUrl, body);
        if (d.ok) return { ok: true };
        if (i < maxRetry - 1) await new Promise(r => setTimeout(r, 800)); // tunggu 0.8s sebelum retry
        else return { ok: false, error: d.error };
      } catch (e) {
        if (i < maxRetry - 1) await new Promise(r => setTimeout(r, 800));
  ...
  const submitSemua = async () => {
    ...
    for (const item of keranjang) {
      ...
      if (result.ok) {
        berhasilIds.push(item.id);
      } else {
        gagalItems.push(item);
      }
    }

    if (gagalItems.length === 0) {
      // Semua berhasil — clear keranjang
      setSubmitStatus("saved");
      setSubmitMsg(`✓ ${berhasilIds.length} item tersimpan!`);
      setKeranjang([]);
      await loadTransaksi(tanggal.slice(0, 7));
      setTimeout(() => setSubmitStatus("idle"), 3000);
    } else {
      // Ada yang gagal — sisakan hanya yang gagal di keranjang
      setKeranjang(gagalItems);
      setSubmitStatus("error");
      setSubmitMsg(`${berhasilIds.length} tersimpan · ${gagalItems.length} gagal (masih di daftar)`);
      await loadTransaksi(tanggal.slice(0, 7));
          ...
          <>
            ...
            <div style={card}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <button onClick={() => {
                  const val = parseFloat((form.jumlah || "0").replace(",", ".")) || 0;
                  const baru = Math.max(0, Math.floor(val) - 1);
                  setForm(f => ({ ...f, jumlah: baru > 0 ? String(baru) : "" }));
                }} style={qBtn}>−</button>
                <input
                  type="text"
                  inputMode="decimal"
                  ...
                  placeholder="0"
                  style={{ ...inp, flex: 1, textAlign: "center", fontSize: 22, fontWeight: 900 }} />
                <button onClick={() => {
                  const val = parseFloat((form.jumlah || "0").replace(",", ".")) || 0;
                  const baru = Math.floor(val) + 1;
                  setForm(f => ({ ...f, jumlah: String(baru) }));
                    ...
                    {barang.map(b => (
                      <div key={b.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0f0f0" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <button onClick={() => setSisaForm(f => ({ ...f, [b.id]: String(Math.max(0, (Number(f[b.id]) || 0) - 0.5)) }))}
                          ...
                          <button onClick={() => setSisaForm(f => ({ ...f, [b.id]: String((Number(f[b.id]) || 0) + 0.5) }))}
